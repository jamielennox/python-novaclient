...
class ServersTest(utils.FixturedTestCase):
    ...
    def test_delete_server_meta(self):
        s = self.cs.servers.delete_meta(1234, ['test_key'])
        self.assert_called('DELETE', '/servers/1234/metadata/test_key')

    def test_set_server_meta(self):
        s = self.cs.servers.set_meta(1234, {'test_key': 'test_value'})
        reval = self.assert_called('POST', '/servers/1234/metadata',
                         {'metadata': {'test_key': 'test_value'}})

    def test_set_server_meta_item(self):
        s = self.cs.servers.set_meta_item(1234, 'test_key', 'test_value')
        reval = self.assert_called('PUT', '/servers/1234/metadata/test_key',
