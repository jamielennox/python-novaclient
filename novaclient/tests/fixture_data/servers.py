...
class Base(base.Fixture):
    ...
    def setUp(self):
        ...
        def put_servers_1234(request, url, headers):
            assert list(request.parsed_body) == ['server']
            fakes.assert_has_keys(request.parsed_body['server'],
        ...
        def post_os_volumes_boot(request, url, headers):
            assert (set(request.parsed_body.keys()) <=
            ...
            data = request.parsed_body['server']
...
class V1(Base):
    ...
    def post_servers(self, request, url, headers):
        ...
        if 'personality' in request.parsed_body['server']:
            for pfile in request.parsed_body['server']['personality']:
                fakes.assert_has_keys(pfile, required=['path', 'contents'])
        if request.parsed_body['server']['name'] == 'some-bad-server':
...
class V3(Base):
    ...
    def post_servers(self, request, url, headers):
        assert set(request.parsed_body.keys()) <= set(['server'])
        fakes.assert_has_keys(request.parsed_body['server'],
