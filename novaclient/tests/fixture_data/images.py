...
class V1(base.Fixture):
    ...
    def setUp(self):
        ...
        def post_images(request, url, headers):
            assert list(request.parsed_body) == ['image']
            fakes.assert_has_keys(request.parsed_body['image'],
        ...
        def post_images_1_metadata(request, url, headers):
            assert list(request.parsed_body) == ['metadata']
            fakes.assert_has_keys(request.parsed_body['metadata'],
                                  required=['test_key'])
