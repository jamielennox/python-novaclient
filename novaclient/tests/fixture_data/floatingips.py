...
class DNSFixture(base.Fixture):
    ...
    def setUp(self):
        ...
        def put_dns_testdomain_entries_testname(request, url, headers):
            fakes.assert_has_keys(request.parsed_body['dns_entry'],
                                  required=['ip', 'dns_type'])
            return 205, headers, request.parsed_body
        ...
        def put_os_floating_ip_dns_testdomain(request, url, headers):
            if request.parsed_body['domain_entry']['scope'] == 'private':
                fakes.assert_has_keys(request.parsed_body['domain_entry'],
                                      required=['availability_zone', 'scope'])
            elif request.parsed_body['domain_entry']['scope'] == 'public':
                fakes.assert_has_keys(request.parsed_body['domain_entry'],
...
class BulkFixture(base.Fixture):
    ...
    def setUp(self):
        ...
        def put_os_floating_ips_bulk_delete(request, url, headers):
            ip_range = request.parsed_body.get('ip_range')
        ...
        def post_os_floating_ips_bulk(request, url, headers):
            params = request.parsed_body.get('floating_ips_bulk_create')
